...
void printCFG() {
    for(auto i : contextFreeGrammar) {
        ...
        for(int j = 0; j < i.second.size(); j++) {
            for(auto k: contextFreeGrammar[i.first][j]) cout << k;
    ...
    for(int i=0; i<n; i++) {
        for(int j=0; j<i; j++) {
            for(auto k: contextFreeGrammar[nonTerminals[i]]) {
                if(k[0] == nonTerminals[j]) replaceProduction(nonTerminals[i], nonTerminals[j]);
                else continue;
            } 
        }
...
int main() {
    ...
    fflush(stdin);
    string s;
    while(getline(inputFile, s)) parseProduction(s);
    cout << "Input CFG - " << endl;
    printCFG();
    removeLeftRecursion();
