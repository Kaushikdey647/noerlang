...
if __name__ == '__main__':
    ...
    nx.draw_networkx_nodes(graph, pos, node_size=2000, node_color='white', edgecolors='black')
    nx.draw_networkx_edges(graph, pos, arrows=True, arrowstyle='->', arrowsize=10)
    nx.draw_networkx_labels(graph, pos, nx.get_node_attributes(graph, 'label'), font_size=6, font_weight='bold')
    nx.draw_networkx_edge_labels(graph, pos, nx.get_edge_attributes(graph, 'label'), font_size=6)
    plt.axis('off')
    plt.title('LR(0) Automaton')
